...
#define BOOST_PROTO2_BINARY_OP(expr, tag)\
    ...
    inline basic_expr<tag, mpl::vector2<ref<basic_expr<LTag, LArgs, LArity> >, typename literal<Right>::type> > const\
    operator expr(basic_expr<LTag, LArgs, LArity> const &left, Right const &right)\
    {\
        basic_expr<tag, mpl::vector2<ref<basic_expr<LTag, LArgs, LArity> >, typename literal<Right>::type> > that = {{left}, proto2::lit(right)};\
        return that;\
    }\
    template<typename Left, typename RTag, typename RArgs, long RArity>\
    inline basic_expr<tag, mpl::vector2<typename literal<Left>::type, ref<basic_expr<RTag, RArgs, RArity> > > > const\
    operator expr(Left const &left, basic_expr<RTag, RArgs, RArity> const &right)\
    {\
        basic_expr<tag, mpl::vector2<typename literal<Left>::type, ref<basic_expr<RTag, RArgs, RArity> > > > that = {proto2::lit(left), {right}};\
        return that;\
    }\
